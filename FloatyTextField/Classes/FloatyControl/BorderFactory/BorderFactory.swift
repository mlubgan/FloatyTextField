...
struct BorderFactory {
    ...
    func createBorder(config: BorderFactoryConfig) -> UIBezierPath {
        ...
        if config.shouldDrawGap {
            bezierPath.move(to: CGPoint(x: FloatyConstants.leadingPadding, y: 0))
        ...
        if config.floatyPaddings == .trailing, config.shouldDrawGap {
            bezierPath.addArc(withCenter: centerPoint1, radius: cornerRadius, startAngle: -(CGFloat.pi / 2 * cornerRadiusConstant), endAngle: 0, clockwise: true)
        } else {
        ...
        if config.floatyPaddings == .leading, config.shouldDrawGap {
            bezierPath.addArc(withCenter: centerPoint4, radius: cornerRadius, startAngle: -CGFloat.pi, endAngle: -CGFloat.pi + cornerRadiusConstant, clockwise: true)
        } else {
